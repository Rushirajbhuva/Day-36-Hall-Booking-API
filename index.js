...
app.get("/AvailableRooms", async function(req,response){
...
app.post("/BookRooms", async function(req,response){
    ...
    if(AvailableRooms === null){
        response.send({Message: "No Rooms available !!"});
    }
...
app.get("/BookedRooms",async function(req,response){
    const result = await client
    .db("B40WE")
    .collection(BookedRooms)
    .find({})
...
app.get("/BookedRooms/:id",async function(req,response){
    const{id}=req.params;
    const result=await client
    .db("B40WE")
    .collection("BookedRooms")
